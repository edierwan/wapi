...
| 0004 | `drizzle/0004_breezy_scarecrow.sql` | 2026-04-26 | ✅ | ✅ | Product master tranche 2: adds `product_status`, `product_media_type`, `product_channel`, `product_channel_sync_status`, `product_bundles`, `product_channel_mappings`, `ref_units`, and richer `products` fields (`slug`, `compare_at_price`, `ai_selling_notes`, `ai_faq_notes`, `tags`). Applied first to `wapi.dev`, then to `wapi`; `scripts/sql/0002_seed_reference_data.sql` rerun on both so `ref_units=11`. |
