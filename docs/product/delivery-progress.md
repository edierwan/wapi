...
- **`/t/{tenantSlug}/settings/storage` tenant page is now business-facing**: the route now renders a Files & Media summary with friendly categories, upload placeholders, recent files, and tenant-safe privacy wording. Provider names, bucket names, tenant UUID prefixes, and raw internal storage paths are no longer shown to tenant members.
...
- 2026-04-27: tenant workspace nav under `/t/[tenantSlug]` is now a left sidebar instead of a horizontal top bar. New files: `src/app/t/[tenantSlug]/layout.tsx`, `src/components/tenant/tenant-sidebar.tsx`, `src/components/tenant/tenant-nav-items.ts`, `src/components/tenant/tenant-page.tsx`. The legacy `TenantSubNav` is now a no-op stub (returns `null`) so existing page imports keep compiling while routes move to the shared page shell. Sidebar sections: Workspace, Communication, Catalog, AI & Growth, Settings (includes a Files & Media item pointing to `/settings/storage`). Active state covers nested detail routes (contact / campaign / inbox conversation pages highlight their parent). Mobile/tablet collapse to a drawer with a floating menu button. Admin sidebar at `/admin` is unchanged. Validation pending for the layout/copy/storage cleanup tranche.
